...
// Cargar categorías en panel lateral
function cargarCategoriasPanel() {
    const panelContent = document.getElementById('categorias-panel');
    ...
    cats.forEach(c => {
        ...
        btn.className = 'panel-categoria-btn';
        btn.innerText = c;
        ...
        btn.onclick = () => {
            ...
            document.querySelectorAll('.panel-categoria-btn').forEach(b => b.classList.remove('active'));
            btn.classList.add('active');
...
function irAInicio() {
    ...
    // Actualizar botón activo en panel
    document.querySelectorAll('.panel-categoria-btn').forEach(b => b.classList.remove('active'));
    const btnTodas = document.querySelector('.panel-categoria-btn');
...
function enfocarBuscador() {
    const buscador = document.getElementById('buscador-app');
    if (buscador) {
        buscador.focus();
        window.scrollTo({top: 0, behavior: 'smooth'});
    }
...
// Buscar productos (app-style)
function buscarProductos() {
    const termino = document.getElementById('buscador-app').value.toLowerCase().trim();
...
function sincronizarContadores() {
    const contadorHeader = document.getElementById('carrito-contador-header');
    const contadorBottom = document.getElementById('carrito-contador-bottom');
    const contadorFlotante = document.getElementById('contador-carrito');
    
    const total = carrito.length;
    
    if (contadorHeader) contadorHeader.textContent = total;
    if (contadorBottom) contadorBottom.textContent = total;
    if (contadorFlotante) contadorFlotante.textContent = total;
}

// Modificar función añadir al carrito para sincronizar contadores
const originalAñadirAlCarrito = añadirAlCarrito;
añadirAlCarrito = function(codigo) {
    originalAñadirAlCarrito(codigo);
    sincronizarContadores();
};

// Modificar función eliminar del carrito para sincronizar contadores
const originalEliminarDelCarrito = eliminarDelCarrito;
eliminarDelCarrito = function(indice) {
    originalEliminarDelCarrito(indice);
    sincronizarContadores();
};

// Modificar función vaciar carrito para sincronizar contadores
...
// Inicializar categorías en panel cuando cargue la página
...
// Event listener para buscador app-style
document.addEventListener('DOMContentLoaded', () => {
    const buscadorApp = document.getElementById('buscador-app');
    if (buscadorApp) {
        buscadorApp.addEventListener('input', buscarProductos);
        buscadorApp.addEventListener('keypress', (e) => {
            if (e.key === 'Enter') {
                buscarProductos();
            }
        });
