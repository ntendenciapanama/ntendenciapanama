...
<<<<<<< HEAD
        ...
        const imagenes = (product.imagenes && product.imagenes.length > 0) 
            ? product.imagenes 
            : Array.from({length: product.totalImagenes}, (_, i) => `images/${product.codigo}/${i+1}.webp`);

        mainImage.src = imagenes[0];
        mainImage.onerror = () => {
            if (mainImage.src.endsWith('.webp')) {
                mainImage.src = mainImage.src.replace('.webp', '.jpg');
            } else if (mainImage.src.endsWith('.jpg')) {
                mainImage.src = mainImage.src.replace('.jpg', '.png');
            }
        };
...
<<<<<<< HEAD
        ...
        imagenes.forEach((imgSrc, idx) => {
            ...
            thumb.src = imgSrc;
            thumb.className = `modal-thumb ${idx === 0 ? "activa" : ""}`;
            thumb.onerror = () => {
                if (thumb.src.endsWith('.webp')) {
                    thumb.src = thumb.src.replace('.webp', '.jpg');
                } else if (thumb.src.endsWith('.jpg')) {
                    thumb.src = thumb.src.replace('.jpg', '.png');
                }
            };
            thumb.onclick = () => {
                selectedIndex = idx;
